...
const DropdownComponent = React.forwardRef<IDropdownRef, SelectDropdownProps<any>>((props, currentRef) => {
  ...
  const performSearch = useCallback(
    (text: string) => {
      ...
      const createSearchFilter = () => {
        if (searchQuery) {
          return (e: any) => {
            const labelText = _get(e, searchField || valueField);
            return searchQuery(text, labelText);
          };
        }

        return (e: any) => {
          const item = _get(e, searchField || valueField)
            ?.toLowerCase()
            .replace(/\s/g, '')
            .normalize('NFD')
            .replace(/[\u0300-\u036f]/g, '');
  ...
  const updateCurrentValue = useCallback(() => {
    const defaultValue =
      typeof inputProps?.value === 'object' ? _get(inputProps?.value, valueField) : inputProps?.value;
    const selectedItem = dataSet.find((e: any) => _isEqual(defaultValue, _get(e, valueField)));
  ...
  const scrollToSelectedIndex = useMemo(
    () =>
      debounce(() => {
        ...
        const defaultValue =
          typeof inputProps?.value === 'object' ? _get(inputProps?.value, valueField) : inputProps?.value;
        const index = _findIndex(state.listData, e => _isEqual(defaultValue, _get(e, valueField)));
  ...
  const renderDropdown = useCallback(() => {
    const isSelected = state.currentValue && _get(state.currentValue, valueField);
    ...
    return (
      <Input
        ...
        value={isSelected ? _get(state.currentValue, valueField) : undefined}
        rightIconAction={{
          iconColor: 'black',
          ...
          <Box>
            {customDropdownItem ? (
              ...
              <Box className={cn('p-3', dropdownItemProps?.className)}>
                <String {...dropdownItemProps?.textItemProps}>{_get(item, valueField)}</String>
              </Box>
            )}
          </Box>
        </TouchableHighlight>
