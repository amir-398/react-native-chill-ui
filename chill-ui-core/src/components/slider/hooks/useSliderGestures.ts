  ...
  useEffect(() => {
    ...
    values.forEach((val, index) => {
      if (val instanceof Animated.Value) {
        ...
        // Initialize with 0, will be updated by listener
        currentValuesRef.current[index] = 0;
      } else {
    ...
    (gestureState: { dy: number; dx: number }) => {
      ...
      if (values.length > 1) {
        if (activeThumbIndexRef.current === 1) {
          // Right thumb cannot go past left thumb + step
          minValue = rawValues[0] + effectiveStep;
        } else {
          // Left thumb cannot go past right thumb - step
          maxValue = rawValues[1] - effectiveStep;
        }
      }
