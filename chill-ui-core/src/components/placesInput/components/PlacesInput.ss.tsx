...
import { AutocompleteDropdownSs } from '@components/AutocompleteDropdown';
import { PlaceSs, PlacesSs, PlacesResponseSs, PlacesInputPropsSs } from '@types';
 ...
 */
export function PlacesInput(props: PlacesInputPropsSs) {
  const {
  ...
  const [places, setPlaces] = useState<PlacesSs[]>([]);
  ...
  const fetchPlaces = useCallback(
    async (input: string) => {
      ...
      try {
        ...
        const data = (await response.json()) as PlacesResponseSs;
        if (data.error) {
          if (onError) {
  ...
  const handlePlaceSelect = useCallback(
    async (item: PlacesSs) => {
      ...
      try {
        ...
        const data = (await response.json()) as PlaceSs;
