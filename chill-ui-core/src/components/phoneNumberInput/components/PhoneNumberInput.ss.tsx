...
import { PhoneNumberInputCountryCodesProps, PhoneNumberInputPropsSs } from '@types';
 ...
 */
export function PhoneNumberInput(props: PhoneNumberInputPropsSs) {
  const {
    allowedCountries,
    customDropdownItem,
    defaultCountry,
  ...
  const DefaultDropdownItemRender = useCallback(
    (item: PhoneNumberInputCountryCodesProps) => (
      <BoxSs style={styles.dropdownItemContainer}>
        <Image source={getFlag(item?.code)} style={styles.flagImage} />
        <StringSs style={styles.countryName}>{language === 'fr' ? item?.fr : item?.en}</StringSs>
        <StringSs>{item?.dial_code}</StringSs>
      </BoxSs>
    ),
    [language],
  );
  ...
  const leftCustomIcon = useMemo(
    () => (
      <BoxSs style={styles.leftIconContainer}>
        {selectedCountry && <Image source={getFlag(selectedCountry.code)} style={styles.flagImageSmall} />}
        <IconSs name="angle-down-solid" size="xs" color="black" />
      </BoxSs>
    ),
    [selectedCountry],
  );
