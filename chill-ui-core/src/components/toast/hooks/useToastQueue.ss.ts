...
import { ToastPropsSs, ToastVariantPropsSs, ToastPositionPropsSs, IconPropsSs, StringPropsSs } from '@types';
...
export const useToastQueue = (options: UseToastQueueOptions = {}) => {
  ...
  const [queue, setQueue] = useState<ToastPropsSs[]>([]);
  const [activeToasts, setActiveToasts] = useState<ToastPropsSs[]>([]);
  ...
  const addToQueue = useCallback(
    (
      ...
      variant: ToastVariantPropsSs = 'info',
      position: ToastPositionPropsSs = 'bottom',
      duration: number = 3000,
    ) => {
      const newToast: ToastPropsSs = {
