...
jest.mock('../components/CustomIcon.hybrid', () => ({
  __esModule: true,
  default: ({ name, color, style, className, ...props }: any) => (
    <div testID={`icon-${name}`} style={style} className={className} data-color={color} {...props}>
      {name}
    </div>
  ),
}));
